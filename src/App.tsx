...
function App() {
  ...
  return (
    <div className="flex flex-col justify-center items-center space-y-4 p-10 min-w-[800px] max-w-[1200px]">
          ...
          <div className="grid grid-cols-3 gap-3">
            {
              todo.filter(item => !item.complete).map((item, index) => (
                <TodoItem key={index} item={item} toggleTodo={() => toggleTodo(item.id)} removeTodo={() => removeTodo(item.id)} />
              ))
            }
          </div>
        </div>
